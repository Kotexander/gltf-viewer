...
use vulkano::{
    ...
    format::Format,
    image::{
        ImageCreateInfo, ImageSubresourceLayers, ImageType, ImageUsage, sampler::Filter,
        view::ImageView,
    },
...
impl Image {
    pub fn new(
        ...
        Self {
            name: image.name().map(From::from),
            vk: ImageView::new_default(vk_image).unwrap(),
        }
    }
}
